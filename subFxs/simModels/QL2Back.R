...
QL2 = function(paras, condition, blockDuration){
  # extract parameter values
  phi_pos = paras[1]; phi_neg = paras[2]; tau = paras[3]; gamma = paras[4]; prior = paras[5]
  
  # initially we estimate nTrial as the max possible nTrial
  nTrial = blockDuration / iti # num of trials 
  junk = as.vector(replicate(round(nTrial / 8 + 1), sample(rewardDelays[[condition]])))
  scheduledWait_ = junk[1:nTrial]
  condition_ = rep(condition, nTrial)
  ...
  # loop over trials
  tIdx = 1
  while(elapsedTime <= blockDuration) {
