expModelFitting = function(ecodeModel, decodeModel){
  #load libraries
  ...
  while(nLoop < 15){
    ...
    if(n > 0){
      ...
      foreach(i = 1 : n) %dopar% {
        ...
        fitFile = sprintf("genData/simModelFitting/%s/%sdb/afit_s%s.RData",
                          encodeModel, decodeModel, thisID)
        ...
        # determine fileName
        fileName = sprintf("genData/simModelFitting/%s/%sdb/s%s", encodeModel,
                           decodeModel, thisID)
        # refit
        # load upper and lower
        tempt = read.csv(sprintf("genData/simModelFitting/%s/%sdb/s%s_summary.txt",
                                 encodeModel, decodeModel, thisID),
                         header = F)
        low= tempt[1:nPara,4]
        up = tempt[1 : nPara,8]
